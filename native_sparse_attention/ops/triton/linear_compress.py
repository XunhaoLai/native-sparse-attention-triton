...
@triton.jit
def linear_compress_fwd_paralleln_paralleld_kernel(
    ...
    x_ptrs = (
        X
        + pid_h * stride_xh 
        + ((x_start + pid_k * BLOCK_OUTPUT_SEQ_SIZE * KERNEL_STRIDE + off_output_seq_size * KERNEL_STRIDE)[:, None] + off_kernel_size[None, :])[:, :, None] * stride_xn
        + off_d[None, None, :] * stride_xd
    )
    x_mask =  (
        (((x_start + pid_k * BLOCK_OUTPUT_SEQ_SIZE * KERNEL_STRIDE + off_output_seq_size * KERNEL_STRIDE)[:, None] + off_kernel_size[None, :]) < x_len)[:, :, None]
        &
...
class LinearCompress_V2(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        x: torch.Tensor,
        ...
        linear_compress_fwd_paralleln_paralleld_kernel[grid](
            x,
            y,
            w,
            cu_seqlens,
...
def linear_compress(
    ...
    Wrapper function for LinearCompress.apply
    """
    y, y_cu_seqlens = LinearCompress_V2.apply(x, w, cu_seqlens, kernel_size, kernel_stride)
    # position embedding as a bias
    if pe is not None:
        assert pe.dtype == x.dtype and pe.device == x.device
        pe = rearrange(pe, "h k d -> h (k d)")
